...
void Clock_ISR_Start(void) 
{
    ...
    Clock_ISR_CLKEN |= Clock_ISR_CLKEN_MASK;
	Clock_ISR_CLKSTBY |= Clock_ISR_CLKSTBY_MASK;
}
...
void Clock_ISR_Stop(void) 
{
    ...
    Clock_ISR_CLKEN &= (uint8)(~Clock_ISR_CLKEN_MASK);
	Clock_ISR_CLKSTBY &= (uint8)(~Clock_ISR_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_ISR_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_ISR_StopBlock(void) 
{
    if ((Clock_ISR_CLKEN & Clock_ISR_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_ISR__CFG3)
        CLK_DIST_AMASK = Clock_ISR_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_ISR_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_ISR__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_ISR_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_ISR_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_ISR_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_ISR_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_ISR_CLKSTBY &= (uint8)(~Clock_ISR_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_ISR_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_ISR_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_ISR_GetSourceRegister();
    uint16 oldDivider = Clock_ISR_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_ISR_CLKEN & Clock_ISR_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_ISR_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_ISR_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Clock_ISR_GetDividerRegister(void) 
{
    return CY_GET_REG16(Clock_ISR_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_ISR_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_ISR_SetModeRegister(uint8 modeBitMask) 
{
    Clock_ISR_MOD_SRC |= modeBitMask & (uint8)Clock_ISR_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_ISR_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_ISR_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_ISR_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_ISR_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_ISR_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_ISR_GetModeRegister(void) 
{
    return Clock_ISR_MOD_SRC & (uint8)(Clock_ISR_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_ISR_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_ISR_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_ISR_GetDividerRegister();
    uint8 oldSrc = Clock_ISR_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_ISR_MOD_SRC |= CYCLK_SSS;
        Clock_ISR_MOD_SRC =
            (Clock_ISR_MOD_SRC & (uint8)(~Clock_ISR_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_ISR_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_ISR_GetSourceRegister(void) 
{
    return Clock_ISR_MOD_SRC & Clock_ISR_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_ISR_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_ISR_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_ISR_PHASE = clkPhase & Clock_ISR_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_ISR_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_ISR_GetPhaseRegister(void) 
{
    return Clock_ISR_PHASE & Clock_ISR_PHASE_MASK;
}
